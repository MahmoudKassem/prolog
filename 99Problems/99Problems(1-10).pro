main() :-
    ...
    myLast(List1, LastElement1), format("~w -> ~w\n", [List1, LastElement1]),
    myLast(List2, LastElement2), format("~w -> ~w\n", [List2, LastElement2]),
    myLast(List3, LastElement3), format("~w -> ~w\n\n", [List3, LastElement3]),
    resetTestLists(List1, List2, List3, List4, List5, List6, List9, List10),

    writeln("#2 last but one element of a list"),
    lastButOne(List1, LastButOneElement1), format("~w -> ~w\n", [List1, LastButOneElement1]),
    lastButOne(List2, LastButOneElement2), format("~w -> ~w\n", [List2, LastButOneElement2]),
    lastButOne(List3, LastButOneElement3), format("~w -> ~w\n\n", [List3, LastButOneElement3]),
    resetTestLists(List1, List2, List3, List4, List5, List6, List9, List10),
    ...
    myLength(List1, Length1), format("~w -> ~w\n", [List1, Length1]),
    myLength(List2, Length2), format("~w -> ~w\n", [List2, Length2]),
    myLength(List3, Length3), format("~w -> ~w\n\n", [List3, Length3]),
    resetTestLists(List1, List2, List3, List4, List5, List6, List9, List10),

    writeln("#5 reverse a list"),
    myReverse(List1, ReversedList1), format("~w -> ~w\n", [List1, ReversedList1]),
    myReverse(List2, ReversedList2), format("~w -> ~w\n", [List2, ReversedList2]),
    myReverse(List3, ReversedList3), format("~w -> ~w\n\n", [List3, ReversedList3]),
    resetTestLists(List1, List2, List3, List4, List5, List6, List9, List10),
    ...
    myFlatten(List1, FlattenedList1), format("~w -> ~w\n", [List1, FlattenedList1]),
    myFlatten(List2, FlattenedList2), format("~w -> ~w\n", [List2, FlattenedList2]),
    myFlatten(List3, FlattenedList3), format("~w -> ~w\n", [List3, FlattenedList3]),
    myFlatten(List7, FlattenedList7), format("~w -> ~w\n", [List7, FlattenedList7]),
    myFlatten(List8, FlattenedList8), format("~w -> ~w\n\n", [List8, FlattenedList8]),
    resetTestLists(List1, List2, List3, List4, List5, List6, List9, List10),

    writeln("#8 remove consecutive duplicates in a list"),
    compress(List1, CompressedList1), format("~w -> ~w\n", [List1, CompressedList1]),
    compress(List2, CompressedList2), format("~w -> ~w\n", [List2, CompressedList2]),
    compress(List3, CompressedList3), format("~w -> ~w\n", [List3, CompressedList3]),
    compress(List9, CompressedList9), format("~w -> ~w\n", [List9, CompressedList9]),
    compress(List10, CompressedList10), format("~w -> ~w\n\n", [List10, CompressedList10]),
    resetTestLists(List1, List2, List3, List4, List5, List6, List9, List10),

    writeln("#9 pack consecutive duplicates of elements into sublists"),
    pack(List1, PackedList1), format("~w -> ~w\n", [List1, PackedList1]),
    pack(List2, PackedList2), format("~w -> ~w\n", [List2, PackedList2]),
    pack(List3, PackedList3), format("~w -> ~w\n", [List3, PackedList3]),
    pack(List9, PackedList9), format("~w -> ~w\n", [List9, PackedList9]),
    pack(List10, PackedList10), format("~w -> ~w\n\n", [List10, PackedList10]),
    resetTestLists(List1, List2, List3, List4, List5, List6, List9, List10),

    writeln("#10 run-length encoding of a list"),
    encode(List1, EncodedList1), format("~w -> ~w\n", [List1, EncodedList1]),
    encode(List2, EncodedList2), format("~w -> ~w\n", [List2, EncodedList2]),
    encode(List3, EncodedList3), format("~w -> ~w\n", [List3, EncodedList3]),
    encode(List9, EncodedList9), format("~w -> ~w\n", [List9, EncodedList9]),
    encode(List10, EncodedList10), format("~w -> ~w\n\n", [List10, EncodedList10]),
    resetTestLists(List1, List2, List3, List4, List5, List6, List9, List10).
...
myLast(List, LastElement) :-
    ...
    List = [_ | Rest], myLast(Rest, LastElement).
...
lastButOne(List, LastButOneElement) :-
    ...
    List = [_ | Rest], lastButOne(Rest, LastButOneElement).
...
elementAt(List, Position, ElementAt) :-
    ...
    Position = 1 ->
        (
            ...
            List = [Element | _], ElementAt = Element
        );
    Position > 1 ->
        (
            List = [] -> ElementAt = false;
            List = [_ | Rest], NextPosition is Position - 1, elementAt(Rest, NextPosition, ElementAt)
        ).
...
myLength(List, Length, Accumulator) :-
    ...
    List = [_ | Rest], NewAccumulator is Accumulator + 1, myLength(Rest, Length, NewAccumulator).

myReverse(List, ReversedList) :-
    myReverse(List, ReversedList, []).
myReverse(List, ReversedList, Accumulator) :-
    List = [] -> ReversedList = Accumulator;
    List = [Element | Rest], NewAccumulator = [Element | Accumulator], myReverse(Rest, ReversedList, NewAccumulator).
...
pack(List, PackedList) :-
    pack(List, PackedList, [], []).
pack(List, PackedList, ConsecutiveDuplicatesList, Accumulator) :-
    List = [] ->
        (
            ConsecutiveDuplicatesList = [] -> PackedList = Accumulator;
            append(Accumulator, [ConsecutiveDuplicatesList], PackedList)
        );
    List = [Element | Rest],
    (
        ConsecutiveDuplicatesList = [] ->
            (
                append(ConsecutiveDuplicatesList, [Element], NewConsecutivDuplicateList),
                pack(Rest, PackedList, NewConsecutivDuplicateList, Accumulator)
            );
        ConsecutiveDuplicatesList = [DuplicateElement | _] ->
        (
            ...
            append(Accumulator, [ConsecutiveDuplicatesList], NewAccumulator),
